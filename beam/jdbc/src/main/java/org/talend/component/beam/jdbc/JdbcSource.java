...
public class JdbcSource extends PTransform<PBegin, PCollection<JsonObject>> {
    ...
    public PCollection<JsonObject> expand(final PBegin input) {
        ...
        final PCollection<JsonObject> apply = input.apply(JdbcIO.<JsonObject> read().withRowMapper(new RecordMapper(builder))
                .withDataSourceConfiguration(config.asBeamConfig()).withQuery(config.query).withCoder(workAroundCoder));
    ...
    public static class WorkAroundCoder extends Coder<JsonObject> {
        ...
        public void encode(final JsonObject value, final OutputStream outStream, final Context context) throws IOException {
        ...
        @Override
        public void registerByteSizeObserver(final JsonObject value, final ElementByteSizeObserver observer) throws Exception {
